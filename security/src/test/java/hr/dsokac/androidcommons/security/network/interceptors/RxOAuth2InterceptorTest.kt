...
class RxOAuth2InterceptorTest {
    ...
    lateinit var interceptor: RxOAuth2Interceptor

    @BeforeEach
    fun setupTests() {
        MockitoAnnotations.initMocks(this)

        interceptor = RxOAuth2Interceptor(authRepository)
    ...
    fun successfulRefreshToken401ErrorSuccessfulRequest() {
        ...
        Mockito.`when`(authRepository.refreshToken()).thenReturn(Completable.complete())
    ...
    fun unsuccessfulRefreshToken() {
        ...
        Mockito.`when`(authRepository.getTokenType()).thenReturn(TOKEN_TYPE)
        Mockito.`when`(authRepository.getRefreshToken()).thenReturn(REFRESH_TOKEN)
        Mockito.`when`(authRepository.refreshToken()).thenReturn(Completable.error(BadRequest()))
